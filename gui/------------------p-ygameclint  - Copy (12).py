...
def uno():
    ...
    while thisloop:
        ...
        if game_state[str(username)]!=0 and unoflag==True:
            popuptext=game_state[str(username)]['drawed']
            unoflag=False
        else:
            popuptext=''
            if game_state[str(username)]==0:
                unoflag=True
        #print(yo)
...
def gameloop():
    ...
    winner=''
    flg=''
    pop_up_text=flash_text(screen,'lol',0,0,size=100)
    while running:#main loop
        ...
        pop_up_text.place()
        if popuptext!='':
            pop_up_text.txt=popuptext
            thrd(pop_up_text)
            prn('gameloop=',popuptext,color='white')
            popuptext=''
        pygame.display.update()
