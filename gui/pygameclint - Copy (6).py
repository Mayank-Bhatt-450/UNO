...
def prn(*a,**k):
    ...
    try:
        ...
        print(f"\x1b[{random.randint(0,8)};{30};{random.randint(43,48)}m",a ,'\x1b[0m')
#########################################################################################
import os
...
def login_view(username,password):
    ...
    if password=='':
        return 'password'
    login_request=f"http://127.0.0.1:8000/login{username},{password}"
    prn(login_request)
    return json.loads( requests.get(login_request).text)
def waiting_view(host):
    login_request=f"http://127.0.0.1:8000/{username}'s"
    return json.loads( requests.get(login_request).text)
def initilize_game(username):
    login_request=f"http://127.0.0.1:8000/{username}_startgame"
    requests.get(login_request)
def create_room_view(username):
    login_request=f"http://127.0.0.1:8000/{username}room"
    requests.get(login_request)
def join_room_view(username,host):
    login_request=f"http://127.0.0.1:8000/join{username},{host}'s"
    return json.loads( requests.get(login_request).text)
#################################################################
#########GAME VARIABLES#################################################
host=''
...
def gameloop():
    ...
    global current,clickedcard,clickflag,action,draw_break,choose_color_flag,color_pt,pos
...
def intro_screen():
    ...
    while running:#main loop
        ...
        text(screen,f'wrong hostid{tem_host}',200,200,size=50,color=(255,0,0))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
            ...
            if join_room_button.event(event):
                ...
                a=join_room_view(username,host)['text']
                if a=='join done':
                    flag='join_room'
                    host=tem_host
                    break
...
def waiting():
    ...
    while running:#main loop
        screen.fill((255,255,255))
        text(screen,'waiting for players...',0,0,size=100)
        responce=waiting_view(host)['players']
        print(responce,type(responce))
        for i in range(len(responce)):
            text(screen,f"{i+1}){responce[i]}",100,100+(100*i),size=70)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
