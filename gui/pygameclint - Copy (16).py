...
def waiting_view(host):
    login_request=f"http://127.0.0.1:8000/{host}'s"
...
def join_room_view(username,host):
    ...
    login_request=f"http://127.0.0.1:8000/join{username},{host}"
    #print(login_request)
    l=requests.get(login_request).text
    #prn(l)
    return json.loads(l )
def leave_waiting_view(username,host):
    login_request=f"http://127.0.0.1:8000/{username},{host}leave_waiting"
    requests.get(login_request)
def leave_game_view(username,host):
    login_request=f"http://127.0.0.1:8000/{username},{host}leave_game"
    requests.get(login_request)
#################################################################
#########GAME VARIABLES#################################################
host=''
...
def uno():
    ...
    pos=username
    while True:
        source =json.loads( requests.get(f'http://127.0.0.1:8000/{host}_startgameview').text)#.replace("\\",'')
        #print(source['text'])
