...
for ii in [(175,78),(605,78),(1046,78),(175,507),(1046,507)]:
    while True:
        ss=ramdom.randint(0,4)
        if ss not in tem_name:
            players_tag[str[i]]=tags_for_players[ss]
            tem_name.append(ss)
            break
player_places={ '2':[(0,78)],
                '3':[(0,78),(871,78)],
                '4':[(0,78),(430,78),(871,78)],
                '5':[(0,78),(871,78),(0,507),(867,507)],
                '6':[(0,78),(430,78),(871,78),(0,507),(867,507)]}
player_name_tag_pos={   '2':[(175,78)],
                        '3':[(175,78),(605,78)],
    ...
    if (x>x0 and y>y0) and (x<x1 and y<y1):
        if clickcheck()==True:
            ...
            if guno_flag==False:
                ...
                guno_flag=False
class other_hand_cards:
    def __init__(self,screen,plr,x,y,l):
            self.screen=screen
            self.plr=plr
            self.x=x
            self.y=y
            self.l=l
            self.xalpha=0
            self.spacing=round(349/l)
            if self.spacing >110:
                self.spacing=110
                self.xalpha=int((self.x+175)-(l/2)*110)
            for i in range(self.l):
                x0=(spacing*i)+xalpha
                y0=placeholder_cards[1]
def gunoturn():
    global game_states,num_of_players,username,screen
    ...
    for i in range(num_of_players):
        ...
        if  plr!=username:
            other_hand_cards(screen,plr,placeholder_cards[0],
                                        placeholder_cards[1],
                            len(game_state[plr]['hand_cards']))
            x+=1
