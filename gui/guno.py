...
screen = pygame.display.set_mode((1200,800))#,pygame.FULLSCREEN)#(1024,768))
...
class entry:
    def __init__(self,screen,x,y,w=100,h=10,text='',
                 ...
                 font=None,boder=2,password=False,outline=(0,0,0)):
        self.screen=screen
...
class button:
    ...
    def place(self):
        ...
        self.fontx=pygame.font.Font(None,self.t)
...
class print_text:
    def __init__(self,screen,text,x,y,font=None,
...
def waiting(win=''):
    ...
    txt=text(screen,'waiting for players...',600,50,size=100,color=(255,255,255),center=True)
    while running:#main loop
        ...
        if win=='':
            ...
            leave_button.place()
            for i in range(len(responce)):
                print_text(screen,f"{i+1}){responce[i]}",357,130+(100*i),size=70,a=True)
            ...
            for i in range(len(responce)):
                print_text(screen,f"{i+1}){responce[i]}",126,130+(100*i),size=70,a=True)
            w_text.place()
        leave_button.place()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
