...
########################################################################
import random
#HOST="http://404bhatt.pythonanywhere.com/"#
...
def login_view(username,password):
    global answer
    if username=='':
        return {'text':'wusername'}
    if password=='':
        return {'text':'wpassword'}
    login_request=f"{HOST}{username},{password}_login"
    try:
...
def waiting_view(host):
    global answer
    login_request=f"{HOST}{host}_wating_room"
    try:
...
def start_game(username):
    global answer
    login_request=f"{HOST}{username}_start_game"
    try:
        requests.get(login_request)
    except:
        network(f"{HOST}{username}_start_game")
def create_room_view(username):
    global answer
    login_request=f"{HOST}{username}_create_room"
    try:
        requests.get(login_request)
    except:
        prn('erroe in create view')
        network(f"{HOST}{username}_create_room")
def join_room_view(username,host):
    global answer
    if host=='':
        return {'text':'host is empty'}
    login_request=f"{HOST}{username},{host}_join_room"
    #print(login_request)
    try:
...
def leave_waiting_view(username,host):
    global answer
    login_request=f"{HOST}{username},{host}_leave_waiting"
    try:
        requests.get(login_request)
    except:
        prn('erroe in create view')
...
def leave_game_view(username,host):
    global answer
    login_request=f"{HOST}{username},{host}_leave_game"
    try:
        requests.get(login_request)
    except:
        prn('erroe in create view')
        network(f"{HOST}{username}_create_room")
def is_joined_view(username):
    global answer
    login_request=f"{HOST}{username}_is_joined"
    print(login_request)
    try:
        l=requests.get(login_request).text
    except:
        prn('erroe in create view')
        network(f"{HOST}{username}_is_joined")
        l=answer
    print(l)
    return json.loads(l )
#################################################################
...
process_show_flag=False
thrd_flag=True
invalid_img=pygame.image.load('invalid_card.png')
kick_img=pygame.image.load('kick page.png')
waiting_img=pygame.image.load('waiting.png')
WINNERwaiting_img=pygame.image.load('WINNER waiting.png')
leave_img=pygame.image.load('leave page.png')
...
player_places={ '2':[(430,78)],
                ...
                '5':[(0,507),(0,78),(871,78),(867,507)],
                '6':[(0,507),(0,78),(430,78),(871,78),(867,507)]}
player_name_tag_pos={   '2':[(605,78)],
                        '3':[(175,78),(1046,78)],
                        '4':[(175,78),(605,78),(1046,78)],
                        '5':[(175,507),(175,78),(1046,78),(1046,507)],
                        '6':[(175,507),(175,78),(605,78),(1046,78),(1046,507)]}
##################################################################################
#username='bhatt1'
def uno():
    ...
    global gameloop_flag,winner,popuptext
    global thrd_flag,process_show_flag
    unoflag=True
    #pos=username
    thisloop=True
    aaa={}
    while thisloop:
        try:
            source =json.loads( requests.get(f'{HOST}{host}_game_state_view').text)#.replace("\\",'')
        except:
            network(f'{HOST}{host}_game_state_view')
            #prn(requests.get(f'{HOST}{host}_game_state_view').text)
        ...
        if game_state[str(username)]['drawed']!=0 and unoflag==True:
            popuptext=game_state[str(username)]['drawed']
            unoflag=False
            #prn(f'uno_{} ')
        else:
            popuptext=''
            if game_state[str(username)]['drawed']==0:
                unoflag=True
        #print(yo)
        ...
        if game_state[str(username)]['turn']=='true':
            ...
            #prn(post)
            try:
                process_show_flag=True
                requests.get(HOST+json.dumps(post)+"_myturn")
                thrd_flag=False
            except:
                thrd_flag=False
                network(HOST+json.dumps(post)+"_myturn")

            #requests.get(HOST+json.dumps(post)+"_myturn")#.replace("\\",'')
...
class invalid_animation:
    ...
    def place(self):
        ...
        self.temp.blit(self.img, (0, 0))
        self.temp.set_alpha(self.alpha)
        self.screen.blit(self.temp, self.pos)
def ineg(a1):
    #a1=invalid_animation(screen,invalid_img,(0,651))
    a1.alpha=150
    #a1.place()
    ...
    while True:
        ...
        if a1.alpha>0:
            a1.alpha-=15
            time.sleep(0.001)
            #print('should show',a1.alpha)
            a1.place()
        else:
            break

def ithrd(a):
    threadz=threading.Thread(target=ineg,args=(a,))
    threadz.daemon=True
    threadz.start()
...
def pile(events):
    ...
    xm=0
    ym=0
    invalid_animation_variable=invalid_animation(screen,invalid_img,(0,651))
    #invalid_animation_variable.alpha=100
    #invalid_animation_variable.place()
    #print(invalid_animation_variable.alpha)
    if current!=['',''] :
    ...
    if events!=():
        ...
        if clickcheck()==True:
            ...
            print(x1,y1)'''
        print(thrd_flag)
        if (x>x0 and y>y0) and (x<x1 and y<y1) and TURN==True and clickedcard!=[-1, []] and thrd_flag==True:
            ...
            try:
                if draw_break==False:
                    if current[1]=='+4'and ((current[1]==clickedcard[1][1])):
                        ...
                        print('action=',action)
                    elif current[1]=='+2' and ((current[1]==clickedcard[1][1])):
                        action=clickedcard
                        print('action=',action)
                    else:
                        ithrd(invalid_animation_variable)

                elif clickedcard[1][1]=='+4' or clickedcard[1][0]=='black':
                    choose_color_flag=True
                elif clickedcard[1][1]=='c'or clickedcard[1][0]=='black':
                    choose_color_flag=True
...
def kick_from_game(plr):
    global screen,host
    prn('',color='pink')
    cancle_button=button(screen,497,434,text='No, Let Him Play',h=37,w=200,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    kick_button=button(screen,703,434,text='Yes, Kik Him Out',h=37,w=200,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
...
def leave_from_game():
    ...
    prn('',color='pink')
    cancle_button=button(screen,497,434,text='No, I wanna Stay',h=37,w=200,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    kick_button=button(screen,703,434,text='Yes, I Want To',h=37,w=200,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    ...
    if flg=='':
        ...
        return True
def gunoturn(events):
    global game_states,username,screen,back_img,host
    #prn(game_states)
    if game_states!={}:
        ...
        if num_of_players!=1:
            ...
            #prn('num_of_players=',num_of_players,
            #"game_states['players']=",game_states['players']
            #,color='green')
            try:
                plr_list=game_states['players']
                for ij in range(len(plr_list)):
                    if plr_list[ij]==username:
                        plr_list=plr_list[ij+1:]+plr_list[:ij]
                        break
            except:
                pass


            for ip in range(num_of_players):
                if num_of_players==1:
                    break

                #prn('ip=',ip,f"num_of_players={num_of_players}",color='light_blue')
                try:
                    plr=plr_list[ip]

                except:
                    break
                #prn(str(num_of_players),player_places[str(num_of_players)],game_states['players'],xx)
                if  plr!=username:

                    placeholder_cards=player_places[str(num_of_players)][xx]
                    placeholder_name=player_name_tag_pos[str(num_of_players)][xx]
                    ...
                    for ii in range(l):
                        if xalpha !=0:
                            x0=(spacing*ii)+xalpha
                        else:
                            x0=placeholder_cards[0]+(spacing*ii)
                        y0=placeholder_cards[1]
                        #prn(f"x0={x0},y0={y0}",color='light_blue')
                        if ii==l-1:
                            if xalpha!=0:
                                pos_is=(x0+110-xalpha,y0)
                            else:
                                pos_is=(x0+110-placeholder_cards[0],y0)
                    ...
                    if xalpha!=0:
                        name_tag_pos=((xalpha)+(pos_is[0]/2))-131,placeholder_name[1]-75
                    else:
                        name_tag_pos=((placeholder_cards[0])+(pos_is[0]/2))-131,placeholder_name[1]-75
                    ...
                    if plr_len<=4:
                        ...
                        size=50
                    fontx=pygame.font.Font('freesans.ttf',size)
...
def link_thread(link):
    ...
    while True:
        try:
            a=requests.get(link).text.replace("\\",'')
            prn(a,color='yellow')
            answer=a
            break
        except:
...
def network(link):
    ...
    while network_v:
        ...
        pygame.display.update()
class flash_text:
    def __init__(self,screen,text,x,y,font='freesans.ttf',size=10,color=(0,0,0,255)):
        self.screen=screen
        self.text=''
        self.x=x
        self.y=y
        ...
        self.alpha=255
    def place(self):
        self.fontx=pygame.font.Font('freesans.ttf',self.size)
        ...
        self.s=self.txt_surf.get_rect().size
        self.screen.blit(self.txt_surf,(self.x-(self.s[0]/2),self.y-(self.s[1]/2)))
def neg(a,t):
    a.text=t
    a.alpha=255
    while True:
...
def thrd(a,t):
    thread=threading.Thread(target=neg,args=(a,t,))
    thread.daemon=True
    thread.start()
    #print('a.text=',a.text)
...
class process_animation:
    ...
    def place(self):
        self.temp = pygame.Surface(self.image_dimension).convert()
        self.temp.blit(self.screen, (-(self.pos[0]-(self.image_dimension[0]/2)),
                                     -(self.pos[1]-(self.image_dimension[1]/2))))
        self.surf =self.rot_center(self.img,self.angle)
        self.temp.blit(self.surf, (0, 0))
        self.temp.set_alpha(self.alpha)
        self.screen.blit(self.temp, (self.pos[0]-(self.image_dimension[0]/2),self.pos[1]-(self.image_dimension[1]/2)))
...
def process_animation_thrd_CALL(a):
    ...
    threads=threading.Thread(target=process_animation_thrd,args=(a,))
    threads.daemon=True
    threads.start()
    print('inn')
def gameloop():
    ...
    global deck,ldeck,screen,hand_cards_list,bg,gameloop_flag,winner,popuptext
    global process_show_flag
    socket()
    gameloop_flag=False
    running=True
    winner=''
    flg=''
    process_i=process_animation(screen,process_img,(600,400))
    pop_up_text=flash_text(screen,'',600,400,size=200)
    while running:#main loop
        event_is=()
        if winner!='':
            flg=winner
            prn(f'gameloop_flg={flg}',color='red')
            running=False
            break
        if gameloop_flag:
            ...
            break
        if winner!='':
            prn(f'gameloop_flg_after break={flg}',color='red')
        ...
        pop_up_text.place()
        process_i.place()
        if process_show_flag==True:
            prn('innnnnnnnnnnnnn')
            process_animation_thrd_CALL(process_i)
            process_show_flag=False#'''
            prn('------innnnnnnnnnnnnn-')
        if popuptext!='':
            #pop_up_text.txt=str(popuptext)
            thrd(pop_up_text,'+'+str(popuptext))
            #prn('gameloop=',popuptext,type(popuptext),color='white')
            popuptext=''
            #print(pop_up_text.txt)
        pygame.display.update()
    if flg=='you have been kicked' or flg=='':
        intro_screen(flg)
    else:
        prn('',color='yellow')
        prn('WAITING',color='red')
        prn('',color='yellow')
        waiting(flg)
...
class entry:
    def __init__(self,screen,x,y,w=100,h=10,text='',
                 ...
                 font='freesans.ttf',boder=2,password=False,outline=(0,0,0)):
        self.screen=screen
    ...
    def place(self):
        ...
        self.box=pygame.draw.rect(self.screen,(255,255,255),(self.x-(self.boder),self.y-(self.boder),self.w+(self.boder*2),self.h+(self.boder*2)),)
        #print('self.bg_color=',self.bg_color)
        pygame.draw.rect(self.screen,self.bg_color,
                         (self.x,self.y,self.w,self.h)
                         )
        self.fontx=pygame.font.Font(self.font,self.h-10)
        ...
        self.w=max(410,self.places.get_width()+25)
...
class button:
    def __init__(self,screen,x,y,text='button',
                color=(255,255,255),

                 w=50,h=10,text_color=(0,0,0),
                 button_type='',boder_bg=(0,0,0)
                 ):
        self.screen=screen
        self.text=text
        self.x=x
    ...
    def place(self):
        ...
        self.fontx=pygame.font.Font('freesans.ttf',self.t)
...
class print_button:
    def __init__(self,screen,x,y,text='button',color=(255,255,255),
        ...
        pygame.draw.rect(self.screen,
                         ...
                         (self.x,self.y
                          ,self.w,self.h))
        self.fontx=pygame.font.Font('freesans.ttf',self.h)
        self.places=self.fontx.render(self.text,True,self.text_color)
        self.screen.blit(self.places,(self.x+((self.w/2)-(self.places.get_width()/2)),
                                      self.y+((self.h/2)-(self.places.get_height()/2))))
        self.w=max(self.places.get_width()+25,self.given_w)
...
class text:
    ...
    def place(self):
        self.fontx=pygame.font.Font('freesans.ttf',self.size)
        self.places=self.fontx.render(self.text,True,self.color)
        self.s=self.places.get_rect().size
        if self.center==True:
            self.screen.blit(self.places,(self.x-(self.s[0]/2),
                                        self.y-(self.s[1]/2)))
        else:
            self.screen.blit(self.places,(self.x,self.y))

class print_text:
    def __init__(self,screen,text,x,y,font='freesans.ttf',
    size=10,color=(0,0,0),a=False):
        self.screen=screen
        self.text=text
        self.x=x
        self.y=y
...
def login(acc=''):
    ...
    prn(acc,'call login')
    try:
        cookies =open('cookies.guno','r')
        user_cookies=cookies.read()
        cookies.close()

        if user_cookies !='':
            #print('lol',user_cookies,'|')
            user_cookies=user_cookies.split('+')
            responce=login_view(user_cookies[0],user_cookies[1])
            try:
                if responce['text']!='wpassword' and responce['text']!='wusername' :
                    username=user_cookies[0]
                    rs=is_joined_view(username)
                    if rs['text']!='False':
                        host=rs['text']
                        waiting()
                    else:
        ...
        pass
    username_entry=entry(screen,397,236,h=50,w=100)
    password_entry=entry(screen,397,378,h=50,password=True)
    login_button=button(screen,392,450,text="Let's LogIn",h=37,w=200,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    singup_button=button(screen,606,450,text='Let Me SingIn First',h=37,w=200,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    flg_log=''
    note_text=text(screen,'',602,560,size=30,color=(255,255,255),center=True)
    running=True
    while running:#main loop
        prn(acc,'call login')
        ...
        for event in pygame.event.get():
            ...
            if password_entry.event(event):
                responce=login_view(username_entry.get(),password_entry.get())
                #prn(responce,color='light_blue')
                if responce['text']=='wusername':
                    username_entry.bg_color=(181,119,119)
                    note_text.text='Invalid Usernam'
                elif responce['text']=='wpassword':
                    note_text.text='Invalid Password'
                    password_entry.bg_color=(181,119,119)
                elif responce['text']!='':
                    username=username_entry.get()
                    cookies =open('cookies.guno','w')
                    prn(str(username_entry.get())+'+'+str(password_entry.get()),color='green')
                    cookies.write(str(username_entry.get())+'+'+str(password_entry.get()))
                    cookies.close()
                    rs=is_joined_view(username)
                    if rs['text']!='False':
                        flg_log='wating'
            ...
            if singup_button.event(event):
                webbrowser.open(HOST+'register', new=2)
            #username='bhatt'
            #break
                #quit()
        else:
...
def intro_screen(msg=''):
    ...
    create_room_button=button(screen,401,289,text="Let's Creat Room",h=37,w=410,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    join_room_button=button(screen,401,555,text="Let's Join room",h=37,w=410,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    log_out=button(screen,998,2,text="LogOut",h=37,w=200,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    room_name_entry=entry(screen,401,491,h=50)
    heading=text(screen,f'WELCOM {username}',600,80,size=100,color=(255,255,255),center=True)
    error_text=text(screen,'',600,724,size=50,color=(255,255,255),center=True)
    running=True
    flag=''
    ...
    while running:#main loop
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
                pygame.quit()
            if room_name_entry.event(event):
...
def waiting(win=''):
    ...
    global screen,username,host
    start_game_button=button(screen,483,700,text="Let's Start The game",h=37,w=200,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    leave_button=button(screen,998,2,text="Leave Game",h=37,w=200,
    color=(255,255,255),
    text_color=(166,54,254),
    button_type='-',
    boder_bg=(255,255,255))
    ...
    flag=''
    msg=''
    w_text=text(screen,f"{win}",927,558,size=100,color=(255,255,255),center=True)
    txtz=text(screen,'waiting for players...',600,50,size=80,color=(255,255,255),center=True)
    while running:#main loop
        ...
        if responce==[]:
            ...
            break
        if win=='':
            start_game_button.x,start_game_button.y=(483,700)
            screen.blit(waiting_img,(0,0))
            if host==username:
                ...
                for i in range(len(responce)):
                    ...
                    list_button.append(
                    print_button(screen,709,128+(100*i),
                    text='kick',w=100,h=50,color=(255,0,0)

                    )
                    )
            leave_button.place()
            for i in range(len(responce)):
                print_text(screen,f"{i+1}){responce[i]}",357,130+(100*i),size=45,a=True)
        else:#WINNERwaiting_img
            screen.blit(WINNERwaiting_img,(0,0))
            if host==username:
                start_game_button.x,start_game_button.y=(249,700)
                start_game_button.place()
                for i in range(len(responce)):
                    list_button.append(
                    print_button(screen,475,128+(100*i),
                    text='kick',w=100,h=50,color=(255,0,0)
                    )
                    )

            for i in range(len(responce)):
                print_text(screen,f"{i+1}){responce[i]}",126,130+(100*i),size=45,a=True)
            w_text.place()
        leave_button.place()
        txtz.place()
        for event in pygame.event.get():
...
kki=0
try:
    update_flag=json.loads(requests.get(f"{HOST}update_status").text)
except:
    pass
if update_flag['text']=='':

    login('start_'+str(kki))
    kki+=1
else:
    update_text=text(screen,update_flag['text'],600,400,size=30,color=(0,0,0)
    ,center=True)
    runnings=True

    while runnings:#main loop
        screen.fill((255,255,255))
        update_text.place()
        pygame.display.update()
        for event in pygame.event.get():
