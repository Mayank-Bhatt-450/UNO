...
def login_view(username,password):
    ...
    try:
        op=json.loads( requests.get(login_request).text.replace("\\",''))
    except:
...
def waiting_view(host):
    ...
    try:
        a=requests.get(login_request).text.replace("\\",'')
    except:
        network(login_request)
        a=answer
    #prn(a)
...
def join_room_view(username,host):
    ...
    #print(login_request)
    try:
        l=requests.get(login_request).text.replace("\\",'')
    except:
        network(login_request)
        l=answer
    #prn(l)
...
def uno():
    ...
    while thisloop:
        try:
            source =json.loads( requests.get(f'{HOST}{host}_game_state_view').text.replace("\\",''))#.replace("\\",'')
        except:
